...
export async function generateItinerary(destination, days, groupType, theme) {
    if (!isAIAvailable()) {
...
export async function generateBudgetPlan(destination, days, people, budgetLevel) {
    if (!isAIAvailable()) {
...
export async function chatTravelAssistant(history, message) {
    if (!isAIAvailable()) {
...
// Fallbacks
function generateMockItinerary(destination, days, group, theme) {
...
function generateMockBudget(destination, days, people, level) {
    const multipliers = { budget: 50, moderate: 120, luxury: 350, backpacking: 30 };
...
export async function generateHotelDetails(hotelName) {
    if (!isAIAvailable()) {
...
export async function generateRouteDetails(origin, destination, mode) {
    if (!isAIAvailable()) {
