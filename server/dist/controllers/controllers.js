...
export const toggleFavorite = async (req, res) => {
    try {
        ...
        if (isFav) {
            user.favorites = user.favorites.filter((name) => name !== placeName);
        }
        else {
...
export const toggleVisited = async (req, res) => {
    try {
        ...
        if (isVisited) {
            // Remove visited
            user.visited = user.visited.filter((name) => name !== placeName);
...
// AI CONTROLLER
// ==========================================
export const generateItineraryEndpoint = async (req, res) => {
    try {
        ...
        const itinerary = await generateItinerary(destination, Number(days), groupType || "Solo", theme || "Adventure");
        res.json({ itinerary });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
export const generateBudgetEndpoint = async (req, res) => {
    try {
        ...
        const budget = await generateBudgetPlan(destination, Number(days), Number(people), budgetLevel || "Moderate");
        res.json({ budget });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
export const chatAssistantEndpoint = async (req, res) => {
    try {
...
export const getHotelDetailsEndpoint = async (req, res) => {
    try {
...
export const getRouteDetailsEndpoint = async (req, res) => {
    try {
